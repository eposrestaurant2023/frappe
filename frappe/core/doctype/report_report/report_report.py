...
import frappe
import frappe.desk.query_report_report
from frappe import _, scrub
from frappe.core.doctype.custom_role.custom_role import get_custom_allowed_roles
from frappe.core.doctype.page.page import delete_custom_role
from frappe.desk.report_reportview import append_totals_row
...
class ReportReport(Document):
	def validate(self):
		"""only administrator can save standard report_report"""
		if not self.module:
		...
		if self.is_standard == "No":
			...
			if self.report_report_type != "ReportReport Builder":
				frappe.only_for("Script Manager", True)

			if frappe.db.get_value("ReportReport", self.name, "is_standard") == "Yes":
				frappe.throw(_("Cannot edit a standard report_report. Please duplicate and create a new report_report"))

		if self.is_standard == "Yes" and frappe.session.user != "Administrator":
			frappe.throw(_("Only Administrator can save a standard report_report. Please rename and save."))

		if self.report_report_type == "ReportReport Builder":
			self.update_report_report_json()
	...
	def on_trash(self):
		if (
			...
			frappe.throw(_("You are not allowed to delete Standard ReportReport"))
		delete_custom_role("report_report", self.name)
	...
	def is_permitted(self):
		...
		custom_roles = get_custom_allowed_roles("report_report", self.name)
	...
	def update_report_report_json(self):
		if not self.json:
			self.json = "{}"
	...
	def export_doc(self):
		...
		if self.is_standard == "Yes" and (frappe.local.conf.get("developer_mode") or 0) == 1:
			export_to_files(
				record_list=[["ReportReport", self.name]], record_module=self.module, create_init=True
			)

			self.create_report_report_py()

	def create_report_report_py(self):
		if self.report_report_type == "Script ReportReport":
			make_boilerplate("controller.py", self, {"name": self.name})
			make_boilerplate("controller.js", self, {"name": self.name})

	def execute_query_report_report(self, filters):
		if not self.query:
			frappe.throw(_("Must specify a Query to run"), title=_("ReportReport Document Error"))
	...
	def execute_script_report_report(self, filters):
		...
		if execution_time > threshold and not self.prepared_report_report:
			self.db_set("prepared_report_report", 1)

		frappe.cache().hset("report_report_execution_time", self.name, execution_time)

		return res

	def execute_module(self, filters):
		# report_report in python module
		module = self.module or frappe.db.get_value("DocType", self.ref_doctype, "module")
		method_name = get_report_report_module_dotted_path(module, self.name) + ".execute"
		return frappe.get_attr(method_name)(frappe._dict(filters))

	def execute_script(self, filters):
		# server script
		loc = {"filters": frappe._dict(filters), "data": None, "result": None}
		safe_exec(self.report_report_script, None, loc)
		if loc["data"]:
			return loc["data"]
		else:
	...
	def get_data(
		self, filters=None, limit=None, user=None, as_dict=False, ignore_prepared_report_report=False
	):
		if self.report_report_type in ("Query ReportReport", "Script ReportReport", "Custom ReportReport"):
			columns, result = self.run_query_report_report(filters, user, ignore_prepared_report_report)
		else:
			columns, result = self.run_standard_report_report(filters, limit, user)
	...
	def run_query_report_report(self, filters, user, ignore_prepared_report_report=False):
		columns, result = [], []
		data = frappe.desk.query_report_report.run(
			self.name, filters=filters, user=user, ignore_prepared_report_report=ignore_prepared_report_report
		)
	...
	def run_standard_report_report(self, filters, limit, user):
		params = json.loads(self.json)
		columns = self.get_standard_report_report_columns(params)
		result = []
		order_by, group_by, group_by_args = self.get_standard_report_report_order_by(params)
		...
		_result = frappe.get_list(
			...
			fields=[
				...
				else ReportReport._format([c[1], c[0]])
				for c in columns
			],
			filters=self.get_standard_report_report_filters(params, filters),
			order_by=order_by,
			group_by=group_by,
			as_list=True,
			limit=limit,
		...
		columns = self.build_standard_report_report_columns(columns, group_by_args)
	...
	def get_standard_report_report_columns(self, params):
	...
	def get_standard_report_report_filters(self, params, filters):
	...
	def get_standard_report_report_order_by(self, params):
		group_by_args = None
		if params.get("sort_by"):
			order_by = ReportReport._format(params.get("sort_by").split(".")) + " " + params.get("sort_order")

		elif params.get("order_by"):
			order_by = params.get("order_by")
		else:
			order_by = ReportReport._format([self.ref_doctype, "modified"]) + " desc"

		if params.get("sort_by_next"):
			order_by += (
				", "
				+ ReportReport._format(params.get("sort_by_next").split("."))
				+ " "
	...
	def build_standard_report_report_columns(self, columns, group_by_args):
		_columns = []
	...
	def toggle_disable(self, disable):
		if not self.has_permission("write"):
			frappe.throw(_("You are not allowed to edit the report_report."))

		self.db_set("disabled", cint(disable))


def is_prepared_report_report_enabled(report_report):
	return cint(frappe.db.get_value("ReportReport", report_report, "prepared_report_report")) or 0


def get_report_report_module_dotted_path(module, report_report_name):
	return (
		frappe.local.module_app[scrub(module)]
		+ "."
		+ scrub(module)
		+ ".report_report."
		+ scrub(report_report_name)
		+ "."
		+ scrub(report_report_name)
	)
